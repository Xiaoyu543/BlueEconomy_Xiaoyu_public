---
title: "BlueEconomy_Biliometrics"
author: "Xiaoyu Liu"
format: html
editor: visual
---
...
**Aims:** Thestudy aims to conduct a visualization analysis of the trends in research outcomes related to the blue economy since its inception, using bibliometric analysis and altmetric analysis. The research objectives include:

1\. Investigating whether the concept has gained more traction and research.

2\. Exploring if there have been shifts in perspectives on the concept, and if different industries have varied feedback.

3\. Examining the variations in driving industries for the blue economy among different countries and whether changes in perceptions exhibit geographical or economic developmental differences.

4\. Assessing the trends and influence of the concept on social media and other online platforms.

## Part 1. Setup

Setup the markdown document and load the packages.
...
Import and organize the datasets from Scopus and Rayyan.

### 2.1 Tidying up

Import raw data exported from Scopus. As there are too many articles, we had to download them in blocks. First, converted them to bibliometric dataframes and merged together.
...
Import and organize the datasets from Rayyan.

```{r}
#| include: false
bibmeta <- read.csv(file = here("Rayyan results/articles.csv"), header = TRUE)

bibmeta <- bibmeta[bibmeta$Included != "No", ] #exclude the articles that we choosed to be excluded

#Now we find the sectors
bib_sector <-subset(bibmeta, select = -c(title, year, month, day, journal, issn, volume, issue, pages, authors, url, doi, language, publisher, location, abstract, keywords, pubmed_id, pmc_id, Included, Classification, View)) #remove unwanted columns

bib_sector <- bib_sector %>%
 mutate(notes = str_replace_all(notes, ",", " "))#Change commas for spaces 
...
#Split sectors into separate columns (called notes 1 and notes 2)
...
#Replace words to group them together
bib_sector <- bib_sector %>%
...
#merge bib_sector with bibmeta, first change the new column names
...
#merge the two olumn to a new column
...
Extract the altimetrics data from the datasets for subsequent altimetric analysis.
...
#Now we will do the same but for sectors separated in rows
...
### 2.3 Import the tidy datasets

If run part 2.1 and 2.2, it is not necessary to run part 2.3. We can skip 2.1 and 2.2 and directly input the organized data from processed data frames.

```{r}
#| include: false
#Import the bibliometrics dataframe
bib <- read.csv(file = here("Processed data frames/bib_as_df.csv"), header=TRUE)

#Import the tidy rayyan screening sets
...
Description analysis using bibliometrics
...
Conduct the altmetrics analysis using rAltmetrics package
...
##Policies
#table(Altmetrics$cited_by_policies_count, useNA = "always") 
...
## Part 5. Explore the research trend

### 5.1 Weighting of countries in promoting research
...
### 5.2 Shifting perceptions

We use the data frame were sectors are grouped in columns for this first part

```{r}
#We will see total shifts in point of view in accordance with year, per industry, and shifts in time per country
...
Shifting perception per industries (using metaEXTRA)
...
See some altimetrics analysis (joining the merged_table with bibmetaEXTRA)
...
Shifting trends per top countries
...
### 5.3 Driving industries in total and per countries
...
See some altimetrics analysis with this
